...
<div class="row gx-5 mt-5" style="margin: 0px">
	...
	<div class="col-12 col-md-12 col-lg-6 mt-5 mb-5">
		<div class="card ">
			<img alt="" src="<c:url value='/template/web/assets/img/ticket/zoo_order_ticket_home.webp'/>">
			<div class="container mt-5 mb-3">
				<div class="card">
					<div class="card-body">
						<h5 class="card-title text-center">Đơn hàng của bạn</h5>

						<!-- Begin Form for Payment -->
						<form action="<c:url value='/dat-hang' />" method="GET">
							<table class="table">
									...
									<c:forEach var="item" items="${cart}">
										...
										<tr class="cart-items-info">
											...
											<td class="cart-item-price">
												<fmt:formatNumber value="${item.ticketPrice}" pattern="#,###.##" /> đ
											</td>
											<td class="cart-item-qty">
												<input type="number" class="btn-update-cart" value="${item.ticketQty}"
													   data-ticket_name="${item.ticketName}" min="1">
											</td>
											<td class="cart-item-close">
												<c:url var="deleteCart" value="/xoa-ve">
													<c:param name="nameTicket" value="${item.ticketName}" />
												</c:url>
												<a href="${deleteCart}" class="btn btn-success text-white">
													<i class="fa-regular fa-circle-xmark delete-cart" data-delete_id="${item.ticketName}"></i> X
												</a>
											</td>
										</tr>
									</c:forEach>
									<tr class="cart-total">
										<td colspan="2">Tổng: <span style="color: #9c9c9c; font-size: 1rem; font-weight: 400">(Tạm tính)</span></td>
										...
										<c:when test="${coupon.couponCondition == 1}">
											<td colspan="6">
												<p style="border-style: dotted;" class="btn btn-success text-white">
													<i class="fa-solid fa-tags"></i> ${coupon.couponCode} -<span>${coupon.couponNumber} %</span>
												</p>
												<c:set var="total_coupon" value="${total * coupon.couponNumber / 100}" />
												<p id="total-coupon">Tổng giảm:
													<fmt:formatNumber value="${total_coupon}" pattern="#,###.##" /> đ
												</p>
											</td>
											<tr class="cart-total">
												<td colspan="2" class="cart-total-title">Tổng sau giảm:</td>
                              ...
                              <fmt:formatNumber value="${coupon.couponNumber}" pattern="#,###.##" /> đ</span>
												</p>
												<c:set var="total_coupon" value="${total - coupon.couponNumber}" />
											</td>
											<tr class="cart-total">
												<td colspan="2" class="cart-total-title">Tổng sau giảm:</td>
												<td colspan="4" id="load-total" class="cart-total-price">
								...
								<c:if test="${empty sessionScope.cart}">
									<tr class="empty-cart">
										<td colspan="5">
											<img src="https://bizweb.dktcdn.net/100/368/281/themes/739953/assets/empty-cart.png?1661855650057"
												 width="310px" alt="Empty Cart">
										</td>
									</tr>
								</c:if>
								</tbody>
								...
								<tr class="m-1">
									<td colspan="6" class="border border-0">
										<input class="form-check-input" type="radio" value="1" name="payment_method" id="flexRadioDefault2" checked>
										<label class="form-check-label" for="flexRadioDefault2">Thanh toán trực tiếp</label>
									</td>
								</tr>
								<tr class="m-1">
									<td colspan="6">
										<input class="form-check-input" type="radio" value="2" name="payment_method" id="flexRadioDefault1">
										<label class="form-check-label" for="flexRadioDefault1">Thanh toán trực tuyến</label>
									</td>
								</tr>
								...
								<tr>
									<td colspan="6" class="border border-0 text-center">
										<button type="submit" class="btn btn-success text-white w-100 p-2">Đặt Hàng</button>
									</td>
								</tr>
							</table>
						</form>
						<!-- End Form for Payment -->
					</div>
				</div>
			</div>
		</div>
